...
pub fn main() !void {
    ...
    if (use_tls) {
        std.log.info("HTTP/2 over HTTPS benchmark server ready on port {d}", .{port});
        std.log.info("TLS with ALPN h2 negotiation enabled for performance testing", .{});
    } else {
        std.log.info("HTTP/2 benchmark server ready on port {d}", .{port});
    }
...
fn monitorPerformance(ctx: *const anyopaque) void {
    ...
    while (true) {
        ...
        if (time_diff_ms > 0) {
            ...
            std.log.info("[HTTP/2] {d} active | {d} req/s ({d} conn/s) | {d} total reqs | Peak: {d} req/s", .{
