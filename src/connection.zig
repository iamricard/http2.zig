...
pub const Connection = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, reader: std.Io.Reader, writer: *std.Io.Writer, comptime is_server: bool) !@This() {
        var self = @This(){
            .allocator = allocator,
            .reader = reader,
            .writer = writer,
    ...
    pub fn process_request(self: *@This(), stream: *DefaultStream.StreamInstance) !void {
        ...
        // TODO: https://ziglang.org/download/0.15.1/release-notes.html#ArrayList-make-unmanaged-the-default
        // Warning: these will both eventually be removed entirely (in reference to `Managed`).
        var buffer = std.array_list.Managed(u8).init(fba.allocator());
...
test "HTTP/2 connection initialization and flow control" {
    ...
    var connection = try Connection.init(allocator, reader, &writer, false); // Use client mode to avoid preface reading
    // Initialize stream
    var stream = try DefaultStream.init(allocator, &connection, 1);
    const preface_written_data = writer.buffered();
    ...
    // TODO: https://ziglang.org/download/0.15.1/release-notes.html#ArrayList-make-unmanaged-the-default
    // Warning: these will both eventually be removed entirely (in reference to `Managed`).
    var encoded_headers = std.array_list.Managed(u8).init(allocator);
    ...
    const headers_written_data = writer.buffered();
    ...
    const sent_data = writer.buffered();
    ...
    try connection.send_data(stream, data_after_reset, false);
    const sent_data_after_window_update = writer.buffered();
...
test "apply_frame_settings test" {
    ...
    var connection = try Connection.init(alloc, reader, &writer, false);
...
test "send HEADERS and DATA frames with proper flow" {
    ...
    var writer: std.Io.Writer = .fixed(&buffer);
    const allocator = arena.allocator();
    var connection = try Connection.init(allocator, reader, &writer, false);
    var stream = try DefaultStream.init(allocator, &connection, 1);
...
test "send RST_STREAM frame with correct frame_type" {
    ...
    const initial_pos = writer.buffered().len;
    // Send RST_STREAM
